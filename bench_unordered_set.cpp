...
// Funzione di stampa
void printRow(const std::string& N, const std::string& us) {
    std::cout << std::left << std::setw(15) << N
              << std::setw(20) << us << std::endl;
}
...
int main() {
    ...
    std::cout << "--- Benchmark: Solo std::unordered_set ---" << std::endl;
    std::cout << "Tempi in NANOSECONDI (ns) MEDI\n" << std::endl;
    std::cout << std::string(35, '-') << std::endl;
    printRow("Operazione", "std::unordered_set");
    std::cout << std::string(35, '-') << std::endl;
    ...
    for (int n : n_values) {
        ...
        auto end = Clock::now();
        double uset_insert_avg_ns = DurationNs(end - start).count() / n;

        volatile bool found; 
        start = Clock::now();
        for (int key : keys_for_hits) {
            found = (us.find(key) != us.end());
        }
        end = Clock::now();
        double uset_hit_avg_ns = DurationNs(end - start).count() / n;
        ...
        double uset_miss_avg_ns = DurationNs(end - start).count() / n;

        std::cout << std::fixed << std::setprecision(3);
        printRow("  Inserimento", std::to_string(uset_insert_avg_ns));
        printRow("  Ricerca (Hit)", std::to_string(uset_hit_avg_ns));
        printRow("  Ricerca (Miss)", std::to_string(uset_miss_avg_ns));
        std::cout << std::string(35, '-') << std::endl;
    }
    return 0;
}